...
class Agent(object):
    ...
    def act(self, observation, reward, done):
        ...
        if len(mycoordinates)>1 and len(enemycoordinates)>1:
            c=0
            diag=[]
            diag.append([mycoordinates[0][0]-5,mycoordinates[0][1]-5])
            for i in observation:
                c+=1
                d=0
                for z in i:
                    ...
                    if [c,d] in enemycoordinates:
                        ...
                        if c>mycoordinates[0][0]+3 and c< (mycoordinates[0][0] + 9):
                            flag=True
                            ...
                            if d>mycoordinates[0][1]+5:
                                ...
                                if flag:
                                    j=12
                        elif d>mycoordinates[0][1] and d<mycoordinates[-1][1]+3:
                            flag=True
        ...
        if len(mycoordinates)>1:
            ...
            collide.append([mycoordinates[0][0]-2,mycoordinates[0][1]+5])
            collide.append([mycoordinates[0][0]-5,mycoordinates[0][1]-2])
            collide.append([mycoordinates[-1][0]+5,mycoordinates[-1][1]])
            
        if len(enemycoordinates)>1 and not flag:
            items=[2,5,0,14,15,16,17]
            
            if collide[0] in wallcoordinates:
            ...
            if collide[1] in wallcoordinates:
                ...
                if 6 in items:
                    items.remove(6)
            if collide[3] in wallcoordinates:
...
if __name__ == '__main__':
    for i in range(100):
        ...
        env.close()
        with open('/Users/rishab/Desktop/gym-master/data.txt','a') as out:
            out.write("\nYour score: %d" % score)
        out.close()
